...
**背景**:
Phase 15〜16で実装したBlendShape FBXエクスポートにおいて、出力されたFBXファイルにBlendShapeアニメーションカーブが含まれない不具合が発見された。調査の結果、以下の原因を特定した：
1. `ExportInternal()`が`ExportModelSettingsSerialize`を未設定のまま`ModelExporter.ExportObject()`を呼び出している
2. `PrepareHumanoidForExport()`および`PrepareTransformCurvesForExport()`がBlendShapeカーブを空リストで初期化し、元クリップのBlendShapeカーブを破棄している
3. `ContextMenuHandler.CreateFbxExportData()`が`PrepareAllCurvesForExport()`を使用せず、独自に空のカーブリストを作成している

**成果物**:
- `ExportModelSettingsSerialize`を使用したFBXエクスポート設定の適用
- BlendShapeカーブが各Prepare系メソッドで保持されるよう修正
- ContextMenuHandlerのエクスポートデータ作成の改善
- 単体テスト・検証テスト
- （条件付き）低レベルAPI（`Autodesk.Fbx`）によるBlendShapeアニメーション直接書き込み

**タスク**:

#### P17-001: ExportModelSettingsSerialize設定の追加
- `FbxAnimationExporter.ExportInternal()`で`ModelExporter.ExportObject(filePath, singleObject, exportOptions)`オーバーロードを使用するよう変更
- `ExportModelSettingsSerialize`で以下を明示的に設定：
  - `ExportBlendShapes = true`
  - `AnimateSkinnedMesh = true`
- 対象ファイル: `FbxAnimationExporter.cs` (line 249)
- 単体テスト作成

#### P17-002: PrepareHumanoidForExportのBlendShapeカーブ抽出修正
- `PrepareHumanoidForExport()`内で`ExtractBlendShapeCurves(humanoidClip, animator)`を呼び出し、BlendShapeカーブをFbxExportDataに含める
- `new List<BlendShapeCurveData>()` → 抽出した実データに置き換え
- 対象ファイル: `FbxAnimationExporter.cs` (line 684)
- 単体テスト作成

#### P17-003: PrepareTransformCurvesForExportのBlendShapeカーブ抽出修正
- `PrepareTransformCurvesForExport()`内で`ExtractBlendShapeCurves(clip, animator)`を呼び出し、BlendShapeカーブをFbxExportDataに含める
- `new List<BlendShapeCurveData>()` → 抽出した実データに置き換え
- 対象ファイル: `FbxAnimationExporter.cs` (line 626)
- 単体テスト作成

#### P17-004: ContextMenuHandler.CreateFbxExportDataの改善
- `CreateFbxExportData()`を`FbxAnimationExporter.PrepareAllCurvesForExport()`を使用するよう変更
- 独自に空リストを作成する実装を廃止し、エクスポーターの統一されたカーブ抽出ロジックを使用
- 対象ファイル: `ContextMenuHandler.cs` (lines 436-469)
- 単体テスト作成

#### P17-005: BlendShape FBXエクスポート検証テスト
- BlendShapeカーブを含むAnimationClipをFBXエクスポートし、出力FBXにBlendShapeアニメーションが含まれることを検証
- Humanoidリグ + BlendShapeの組み合わせでも正しく出力されることを検証
- ContextMenuHandler経由のエクスポートフローの統合テスト

#### P17-006: （条件付き）低レベルAPIによるBlendShapeアニメーション直接書き込み
- **実施条件**: P17-001の修正後も`ModelExporter.ExportObject()`でBlendShapeアニメーションが出力されない場合
- `Autodesk.Fbx`名前空間（`FbxAnimStack`, `FbxAnimLayer`, `FbxAnimCurve`等）を使用してBlendShapeアニメーションカーブをFBXに直接書き込む
- `調査メモ_P12-001`のセクション5.2（代替アプローチ）に基づく実装
- 単体テスト作成
...
| ModelExporterがBlendShapeアニメーション非対応の可能性 | 高 | ExportModelSettingsSerializeでExportBlendShapes=trueを設定。それでも不可の場合は低レベルAPI（Autodesk.Fbx）で直接書き込み（Phase 17で対応） |
...
### 8.4 BlendShape FBXエクスポート修正（Phase 17）

1. `ExportModelSettingsSerialize`で`ExportBlendShapes`と`AnimateSkinnedMesh`が明示的に有効化されていること
2. `PrepareHumanoidForExport()`でBlendShapeカーブが破棄されず正しく抽出・保持されること
3. `PrepareTransformCurvesForExport()`でBlendShapeカーブが破棄されず正しく抽出・保持されること
4. `ContextMenuHandler.CreateFbxExportData()`が統一されたカーブ抽出ロジックを使用すること
5. BlendShapeカーブを含むAnimationClipのFBXエクスポートで、出力FBXにBlendShapeアニメーションが含まれること
6. Humanoidリグ + BlendShapeの組み合わせで正しくエクスポートされること
7. 単体テスト・検証テストが全てパスすること
...
| 1.2 | 2026-01-27 | Phase 17（BlendShape FBXエクスポート修正）を追加。FBXエクスポート時にBlendShapeアニメーションが出力されない不具合の対応タスク（P17-001〜P17-006）を定義。ExportModelSettingsSerialize設定追加、Prepare系メソッドのBlendShapeカーブ抽出修正、ContextMenuHandler改善、低レベルAPI対応（条件付き）。リスク・完了基準を更新 |
